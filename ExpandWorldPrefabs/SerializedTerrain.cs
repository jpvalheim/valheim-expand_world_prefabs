...
internal static class SerializedTerrain
{
    ...
    TerrainOp.Settings settings)
  {
    ...
    var state = TerrainState.Load(storedBefore, build.m_baseHeights, build.m_baseMask, center, Width, Scale);
    var changed = state.Apply(pos, settings);
    var paintOnly = settings.m_paintCleared && !settings.m_level && !settings.m_raise && !settings.m_smooth;
    if (paintOnly && !changed)
      return true;
    ...
    compiler.Set(ZDOVars.s_TCData, Utils.Compress(bytes));
  ...
  internal sealed class TerrainState
  {
    ...
    private TerrainState(IReadOnlyList<float> baseHeights, Color[] baseMask, Vector3 center, int width, float scale)
    {
      ...
      Array.Copy(baseMask, PaintMask, count);
    }
      ...
      Color[] baseMask, Vector3 center, int width, float scale)
    {
      ...
      var state = new TerrainState(baseHeights, baseMask, center, width, scale);
    ...
    internal bool Apply(Vector3 pos, TerrainOp.Settings settings)
    {
      ...
      var sampledHeights = new float[BaseHeights.Count];
    ...
    private bool Level(Vector3 pos, float radius, bool square, float[] sampledHeights)
    {
      ...
      for (var y = centerY - extent; y <= centerY + extent; y++)
      {
        for (var x = centerX - extent; x <= centerX + extent; x++)
        {
          ...
          changed |= !ModifiedHeight[index] || level != LevelDelta[index] || SmoothDelta[index] != 0f;
    ...
    private bool Raise(Vector3 pos, float radius, float delta, bool square, float power, float[] sampledHeights)
    {
      WorldToVertex(pos, false, out var centerX, out var centerY);
      var localY = pos.y - Center.y;
      var scaledRadius = radius / Scale;
    ...
    private bool Smooth(Vector3 pos, float radius, bool square, float power, float[] sampledHeights)
    {
      WorldToVertex(pos, false, out var centerX, out var centerY);
      ...
      for (var y = centerY - extent; y <= centerY + extent; y++)
      {
        for (var x = centerX - extent; x <= centerX + extent; x++)
        {
          ...
          changed |= !ModifiedHeight[index] || smooth != SmoothDelta[index];
    ...
    private bool Paint(Vector3 pos, TerrainOp.Settings settings, float[] sampledHeights)
    {
      var worldPos = pos;
      worldPos.x -= 0.5f;
      worldPos.z -= 0.5f;
      ...
      for (var y = centerY - extent; y <= centerY + extent; y++)
      {
        for (var x = centerX - extent; x <= centerX + extent; x++)
        {
          ...
          changed |= !ModifiedPaint[index] || !Same(PaintMask[index], color);
    ...
    private float CurrentHeight(int index) => Mathf.Clamp(
      BaseHeights[index] + LevelDelta[index] + SmoothDelta[index],
      BaseHeights[index] - LevelLimit, BaseHeights[index] + LevelLimit);
